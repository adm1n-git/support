...
def validate_json_schema(instance_path, schema_path):
    ...
    try:
        ...
        logger.critical(f"Validation Error - {jsonschema._utils.format_as_index(container='', indices=err.relative_path)}");
    except jsonschema.exceptions.SchemaError as err:
        logger.critical(f"Schema Error - {err.message}");
        logger.critical(f"Schema Error - {jsonschema._utils.format_as_index(container='', indices=err.relative_path)}");
    sys.exit(1);
